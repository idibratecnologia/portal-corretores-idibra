...
import { CheckCircle2, XCircle, Loader2, Calendar, MapPin, BadgeCheck } from 'lucide-react'
        ...
        {loading ? (
          ...
          <>
            <div className="bg-green-600 text-white px-6 py-5 text-center">
              ...
              <p className="font-bold text-lg">Participação válida</p>
            </div>
            <div className="p-6 space-y-3 text-sm">
              <Linha label="Participante" valor={res.nome} forte />
              {res.creci && <Linha label="CRECI" valor={res.creci} />}
              <div className="flex items-start gap-2 text-gray-700"><BadgeCheck className="w-4 h-4 text-green-600 mt-0.5" /><div><p className="text-gray-400 text-xs">Evento</p><p className="font-medium">{res.evento}</p></div></div>
              <div className="flex items-start gap-2 text-gray-700"><Calendar className="w-4 h-4 text-green-600 mt-0.5" /><div><p className="text-gray-400 text-xs">Data</p><p className="font-medium">{res.data}</p></div></div>
