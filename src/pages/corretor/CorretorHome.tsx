...
export function CorretorHome() {
  ...
  const statValues = [
    ...
    `${totalParticipacoes * 10} pts`, // placeholder — replace with real score from backend
  ]
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-white leading-tight">
              {corretor?.nome?.trim().split(' ')[0] ?? 'Olá'}, seja bem-vindo!
            </h1>
                ...
                <div className="relative h-40 overflow-hidden">
                  {evento.banner_url ? (
                    ...
                    <div className={cn('w-full h-full relative overflow-hidden', bannerCfg.gradient)}>
                      ...
                      />
                      <div className={cn('absolute -top-6 -right-6 w-24 h-24 rounded-full blur-2xl opacity-30', bannerCfg.dotColor)} />
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                        <BannerIcon className={cn('w-10 h-10', bannerCfg.iconColor)} strokeWidth={1.5} />
                        <span className={cn('text-[9px] font-bold uppercase tracking-[0.25em] opacity-60', bannerCfg.iconColor)}>{evento.tipo}</span>
                      </div>
                    </div>
                  )}
                  {/* Overlay gradient */}
